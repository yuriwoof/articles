...
また、edit など tools を指定することで、カスタム チャットモードの内容の沿ったディレクトリ構造や JSON ファイルを生成することも可能です。下記は東日本リージョンのみにリソースの展開を限定するポリシーの生成を行って、ディレクトリ構造などカスタム チャットモードの指示通り生成されています。
