---
title: "Azure Arc 対応サーバーで Hybrid Runbook Worker を使ってマシン内の PowerShell スクリプトを実行する"
emoji: "🤖"
type: "tech"
...
オンプレミスや他のクラウド環境にあるサーバーに対して、Azure から自動化タスクを実行したいケースは多いと思います。Azure Automation の Hybrid Runbook Worker を使用すると、Azure Arc 対応サーバー上で直接 PowerShell スクリプトを実行できます。

この記事では、Azure Arc 対応サーバーに Hybrid Runbook Worker をセットアップし、PowerShell スクリプトを実行する手順を解説します。

## Hybrid Runbook Worker とは

Hybrid Runbook Worker [^1] は、Azure Automation の機能の一つで、以下の特徴があります。
...
- Azure Arc サーバーに System Assigned Managed Identity が有効化されていること
...
ローカルスクリプトを実行するための Runbook を作成します。

1. Automation アカウントの"プロセス オートメーション"→"Runbook"を選択
2. "+ 作成"を選択
3. 以下の情報を入力
   - 名前: `Execute-LocalScript`
...
Runbook を GitHub または Azure DevOps と連携することで、コードのバージョン管理と自動同期が可能になります [^4]。
...
Azure Arc 対応サーバーと Hybrid Runbook Worker を組み合わせることで、オンプレミス環境にある Windows サーバー上で PowerShell スクリプトを Azure から実行できるようになります。

これにより、以下のようなシナリオで活用できます。
