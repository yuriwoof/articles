...
入れ子になった仮想マシンとは、仮想マシンの中で仮想マシンを動作させるため、ハイパーバイザがインストールされた仮想マシンを指します。[^1]

なお、入れ子になった仮想マシンについては、テストや評価目的で利用することが推奨されています。[^2]

### 方法1 az vm repair create コマンド

手軽に作る方法としては、仮想マシンの復旧を目的とした Azure CLI のコマンド az vm repair[^3] を使う方法があります。
...
:::message alert
"vm-repair" が "1.0.0b1" の場合、Hyper-V が有効化されない問題が報告されています。[^4]
...
Azure Quickstart Templates[^5] には様々なユースケースに沿った ARM/Bicep テンプレートが含まれています。

入れ子になった仮想マシンの作成については、こちらのテンプレートです。[^6]
...
実施している内容としては、Desired State Configuration (DSC)[^7] 拡張機能を使って、
以下の役割を有効化しています。
...
次にカスタムスクリプト拡張機能にて、hvhostsetup.ps1 を実行し、Final configuration に記載のある設定を行っている模様。[^9]
Hyper-V ホストは２つの NIC を持ち、片方は NAT 用、もう片方は Hyper-V 仮想マシン側のサブネットに足を出しています。[^10]
...
[^1]: [入れ子になった仮想化の概要](https://learn.microsoft.com/ja-jp/virtualization/hyper-v-on-windows/user-guide/nested-virtualization)
[^2]: [Hyper-V VM 上の Hyper-V VM](https://learn.microsoft.com/ja-jp/virtualization/hyper-v-on-windows/user-guide/nested-virtualization#hyper-v-vms-on-hyper-v-vms)
[^3]: [az vm repair コマンド](https://learn.microsoft.com/ja-jp/cli/azure/vm/repair?view=azure-cli-latest)
...
[^6]: [nested-vms-in-virtual-network サンプル](https://learn.microsoft.com/en-us/samples/azure/azure-quickstart-templates/nested-vms-in-virtual-network/)
[^7]: [Desired State Configuration (DSC) 拡張機能](https://learn.microsoft.com/ja-jp/azure/virtual-machines/extensions/dsc-template)
...
[^9]: [hvhostsetup.ps1](https://github.com/Azure/azure-quickstart-templates/blob/master/demos/nested-vms-in-virtual-network/hvhostsetup.ps1) と [Final configuration](https://github.com/Azure/azure-quickstart-templates/tree/master/demos/nested-vms-in-virtual-network#final-configuration)
